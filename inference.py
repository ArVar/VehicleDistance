...
parser.add_argument('-c', '--config',
    default='yolact_plus_base_config', type=str,
    help='Config for specific model to load.')
parser.add_argument('-fn','--fast_nms', default=True, type=bool,
                    help='Whether to use a faster, but not entirely correct version of NMS.')
parser.add_argument('-s','--seed', default=9999, type=int,
                        help='The seed to pass into random.seed. Note: this is only really for the shuffle and does not (I think) affect cuda stuff.')
parser.add_argument('-i','--id', default=0, type=str,
                        help='id which will determine on which feed to work with')
